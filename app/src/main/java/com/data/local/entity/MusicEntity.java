 ...
 * <p>
 */
@Entity(tableName = "MusicEntity")
public class MusicEntity implements Comparable<MusicEntity> {
    ...
    @Override
    public int compareTo(MusicEntity u) {
        if (getReleaseDate() == null || u.getReleaseDate() == null) {
            return 0;
        }

        try {
            Date dateLeft = df.parse(getReleaseDate());
            Date dateRight = df.parse(u.getReleaseDate());
